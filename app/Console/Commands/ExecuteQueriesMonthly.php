...
class ExecuteQueriesMonthly extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'queries:execute-monthly';
     ...
     * @var string
     */
    protected $description = 'Execute scheduled queries monthly';
    ...
    {
        $rows = DB::table('exception_process')->where('frequency', 'month')->get();
        ...
        foreach ($rows as $row) {
            ...
            if (isset($row->data_source)) {
                ...
                if (($row->data_source == 'T24') ||  ($row->data_source == 'Imal')) {
                    $results = DB::select(DB::raw($sql));
                }
                // Check if the 'data_source' is 'Oracle132'
                elseif (($row->data_source == 'oracle132')) {
        ...
        return Log::info('Scheduled queries executed successfully (monthly).');
